...
def test_process_and_next(filling_pipe: FillingPipe,
                          execution_context: ExecutionContext, monkeypatch) -> None:
    """
...
def test_extract_data_basic(filling_pipe: FillingPipe) -> None:
    ...
    # The second time the value is None as it has already been extracted
    # (default = None is returned).
    assert filling_pipe.extract_data('test_data') is None
...
def test_extract_data_with_required(filling_pipe: FillingPipe, default) -> None:
    ...
    with pytest.raises(YAMLSyntaxException,
                       match='The field "test_data" is required for the pipe of type FillingPipe'):
