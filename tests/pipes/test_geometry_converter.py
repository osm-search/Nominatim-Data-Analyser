from nominatim_data_analyser.core.pipes import GeometryConverter
...
def test_process_geometry_converter(geometry_converter: GeometryConverter) -> None:
    ...
    assert data_result['geometry_holder'].coordinates[0] == 10
    assert data_result['geometry_holder'].coordinates[1] == 15


def test_process_geometry_converter_none_holder(geometry_converter: GeometryConverter) -> None:
    """
        Test the process() method of the GeometryConverter with one data
        containing a None value for the geometry_holder.
    """
