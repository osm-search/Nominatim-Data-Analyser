...
from nominatim_data_analyser.core.dynamic_value.resolver import (
...
def test_is_resolvable(variable: Variable) -> None:
    ...
    """
    assert is_resolvable(variable) is True
    assert is_resolvable([20, variable, 20]) is True
    assert is_resolvable((20, variable, 20)) is True
    assert is_resolvable({variable: 20, 'key': 'value'}) is True
    assert is_resolvable({20: variable, 'key': 'value'}) is True
    assert is_resolvable({20: 20, 'key': 'value'}) is False
    assert is_resolvable((20, 20, 20)) is False
    assert is_resolvable([20, 20, 20]) is False
    assert is_resolvable(20) is False
...
def test_contains_dynamic_value(variable: Variable) -> None:
    ...
    """
    assert _contains_dynamic_value([20, variable, 20]) is True
    assert _contains_dynamic_value([20, 20, 20]) is False
...
def test_is_dynamic_value(variable: Variable) -> None:
    ...
    """
    assert _is_dynamic_value(variable) is True
    assert _is_dynamic_value(20) is False
